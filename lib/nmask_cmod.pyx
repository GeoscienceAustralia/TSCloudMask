...
def spatial_filter_v2(onescene):
    ...
    newtsmask = <DTYPEINT8_t *> malloc(pnum*sizeof(char))
    
    for y in range(irow):
        for x in range(icol):
            idx = y*icol + x
            tsmask[idx] = onescene[y, x]
            newtsmask[idx] = onescene[y, x]
...
def spatial_filter_shadow(onescene):
